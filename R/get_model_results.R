...
#' }
get_formula <- function(x, model_index = 1) {
  cli::cli_h1("Formula")
...
#' }
get_residuals <- function(x, model_index = 1) {
...
#' get_residuals(mod_df, 1)
#' }
get_test_outliers <- function(x, model_index = 1) {
...
#' }
get_test_dispersion <- function(x, model_index = 1) {
...
#' }
get_test_zero_inflation <- function(x, model_index = 1) {
  cli::cli_h1("Model zero inflation")
  x[model_index,]$mod_zero_inflation
}

#' Get model object from a dataframe
#'
#' @param x A dataframe of models from create_models()
#' @param model_index Row number from x to get the formula from (optional)
#'
...
#' @examples
#' \dontrun{
#' get_model_object(mod_df, 1)
#' }
get_model_object <- function(x, model_index = 1) {
  x[model_index,]$model[[1]]
}